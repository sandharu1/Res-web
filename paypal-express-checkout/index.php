...
if(isset($_SESSION["cart_products"])) //Post Data received from product list page.
{
	...
	$i = 0;
		...
		$results = $mysqli->query("SELECT itemName, item_cost FROM products WHERE item_id='$product_code' LIMIT 1");
		$obj = $results->fetch_object();
		
        $paypal_data .= '&L_PAYMENTREQUEST_0_NAME'.$i.'='.urlencode($obj->itemName);
        $paypal_data .= '&L_PAYMENTREQUEST_0_NUMBER'.$i.'='.urlencode($cart_itm["product_code"]);
        $paypal_data .= '&L_PAYMENTREQUEST_0_AMT'.$i.'='.urlencode($obj->item_cost);		
		$paypal_data .= '&L_PAYMENTREQUEST_0_QTY'.$i.'='. urlencode($cart_itm["product_qty"]);
        
		// item price X quantity
        $subtotal = ($obj->item_cost*$cart_itm["product_qty"]);
		
        //total price
        $ItemTotalPrice = $ItemTotalPrice + $subtotal;
		
		//create items for session
		$paypal_product['items'][] = array('itm_name'=>$obj->itemName,
											'itm_price'=>$obj->item_cost,
	...
	$padata = 	'&METHOD=SetExpressCheckout'.
				...
				'&LOGOIMG=http://artsy.net16.net/img/logo1.png'. //site logo
